...
const ping = async function (url, interval = 30000, logging = true, flipDM = false, noURLFix = false) {
    ...
    if (!interval)
        interval = 30000;
    if (!url)
        return console.log(`[🏓 @hystleria/pinger] You must specify a URL.`);
...
const dping = async function (webhooktoken, webhookid, url, interval, flipDM = false, noURLFix = false) {
    let count = 1;
    if (!webhooktoken)
        return console.log(`[🏓 @hystleria/pinger] You must specify a webhook token. If you don't want Discord logging, use the .ping method.`);
    if (!webhookid)
        return console.log(`[🏓 @hystleria/pinger] You must specify a webhook id. If you don't want Discord logging, use the .ping method.`);
    if (!interval)
        interval = 30000;
    if (!url)
        return console.log(`[🏓 @hystleria/pinger] You must specify a URL.`);
    if (interval < 10000)
        return console.log(`[🏓 @hystleria/pinger] Due to Discord's API rate limiting, you cannot send more than one request in under 10 seconds. Try using an interval of 10000 or more.`);
    if (!url.startsWith('http://') && !url.startsWith('https://') && !noURLFix) {
        url = `https://${url}`;
    }
    if (URLValidity(url) !== true || encodeURIComponent(url).includes("%3C" || "%3E" || "%20")) {
